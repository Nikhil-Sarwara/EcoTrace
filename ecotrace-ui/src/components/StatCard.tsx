...
const StatCard: React.FC<StatCardProps> = ({ label, value, unit, trend, color }) => {
  const colors = {
  ...
  return (
    <div className="bg-white p-8 rounded-[2rem] border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow group">
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${colors[color]}`}>
        {label.includes('Carbon') ? '🌱' : label.includes('Target') ? '🎯' : '🌳'}
      </div>
